...
- [x] GĐ3 (phần chạy thật): **hoàn thành 08/07 tối** — toàn bộ DoD GĐ3 đạt
  - ✅ 08/07 (tối): **benchmark RQ4 hoàn tất** (360 call, $0.112, 0 lỗi parse, deterministic 100% qua 3 lặp): hit-rate **100% ở cả 3 điều kiện** (orig/blur/pixel) → masking không làm giảm khả năng định vị; IoU giảm nhẹ trên màn PII (c01 0.764→0.709, c03 0.806→0.730), màn không mask giữ nguyên (đối chứng). Số liệu: `masking/generated/grounding-results.csv` (+ `raw-calls.jsonl` audit). ⚠️ Attempt 1 sai do harness chấm sai chuẩn tọa độ Qwen (0–1000) + 3 item dưới fold — đã sửa, ghi `docs/pilot-model-cost.md` mục 8.
  - ✅ 08/07 (tối): key OpenRouter đã cấu hình lại trên máy Windows (`tests-vlm/.env`; ⚠️ key dán qua chat AI — xoay key sau thực nghiệm); pilot xác minh PASS (VLM đếm đúng 12/12 dòng — fix viewport có hiệu lực)
  - ✅ 08/07 (tối): **RBAC VLM trên seeded build: phát hiện 5/5** — merge master vào `rq3-seeded-bugs` (đồng bộ viewport fix + harness, commit `4daf158`), chạy qua harness label `rq3-seeded-vlm-verify`: R1–R5 **fail đúng thiết kế**, R6–R8 pass (không báo động giả) — đối xứng hoàn toàn với locator suite. Token/cost đo end-to-end: 11 AI call, 22 674 in + 746 out, **$0.0052/run**, 56.5s. → RQ3 detection: cả 2 phương pháp 5/5 ở lần chạy verify; số liệu chính thức (lặp 5) chạy ở GĐ4.
  - ✅ 08/07: token/cost đã vào harness (`harness/token-log.mjs` + cột mới trong `matrix-runs.csv`: `run_ai_calls, run_prompt_tokens, run_completion_tokens, run_ai_ms, run_cost_usd`; giá ghim $0.20/$0.88 per M theo `docs/pilot-model-cost.md` mục 6). Parser unit-test PASS; harness self-test locator v0 18/18, CSV migrate header cũ tự động. Cột token chỉ điền cho method=vlm (đọc `tests-vlm/midscene_run/log/*.log`, đo phần append trong từng run).
- [~] GĐ4: Thực nghiệm + phân tích (07/09) — **phần chạy tự động xong sớm 09/07**
  - ✅ 09/07: **Ma trận chính thức RQ1** (label `gd4-main`, 2×4×5, 720 dòng): locator v0 18/18, v1 18/18, v2 **9/18**, v3 **12/18**; VLM **18/18 trên cả 4 biến thể**, 20/20 run, flakiness=0 cả hai phương pháp. VLM ~93 call, ~232k token, **$0.050/run** (tổng ~$1.0); wall-clock VLM ~7–10 phút/run vs locator ~3s (V0/V1) / ~32s (V2/V3 do test fail chờ timeout).
  - ✅ 09/07: **RQ3 chính thức** (label `gd4-rbac-clean` + `gd4-rbac-seeded`, 2 phương pháp × 5 lặp mỗi build): clean build cả hai 8/8×5 (0 báo động giả); seeded build cả hai **R1–R5 fail 5/5, R6–R8 pass 5/5** → detection rate 100% cho CẢ locator lẫn VLM, deterministic. VLM RBAC $0.0052/run.
  - ✅ 09/07: **RQ2 bước khách quan xong** (phương án lai, chi tiết `results/rq2-maintenance.md`): sửa tối thiểu locator trên branch riêng — V2 = **9 test / 24 LOC** (`rq2-fix-v2`, commit `bc5f071`), V3 = **6 test / 22 LOC** (`rq2-fix-v3`, commit `3eb8809`), cả hai xác nhận 18/18; V1 = 0; VLM = 0 test / 0 LOC trên cả 3 biến thể. Chỉ số **thời gian**: đề cương (chưa nộp) đã sửa 09/07 theo hướng "quy trình bảo trì chuẩn hóa tự động" → thời gian = thời gian agent phục hồi suite, sẽ đo trên branch `rq2-agent-v2/v3`.
  - ✅ 09/07: **Phân tích + biểu đồ xong**: `harness/make-figures.py` sinh 5 hình (`results/figures/`: RQ1 pass-rate, RQ1 time/cost, RQ2 maintenance, RQ3 detection, RQ4 hit-rate/IoU) + bảng tổng hợp `results/analysis-summary.md` từ CSV chính thức — tái lập bằng `python3 harness/make-figures.py` (cần matplotlib; máy Mac dùng `python3`, không có alias `py`).
  - ✅ 09/07: **Nháp Chương 4 xong**: `docs/bao-cao/chuong-4-ket-qua.md` (thiết lập, kết quả 4 RQ, thảo luận trade-off, 6 mục Threats to Validity; các chỗ ⚠️ TODO chờ quyết định GVHD/chèn vào template khoa).
  - ✅ 09/07 (chiều): **đề cương .docx cập nhật cách đo RQ2** (quy trình bảo trì chuẩn hóa tự động — AI agent sửa tối thiểu, đo test/LOC/thời gian phục hồi; đoạn 27 + 80 của .docx; đề cương chưa nộp nên sửa trực tiếp). Tài liệu đồng bộ: `CLAUDE.md`, `results/rq2-maintenance.md` mục 4, nháp Chương 4 mục 4.3/4.7. Nội dung trình bày với GVHD (5 mục + bảng "sau buổi gặp"): **`docs/gvhd-trao-doi.md`**.
  - ✅ 09/07 (chiều): **đo thời gian phục hồi RQ2 xong — GĐ4 hoàn tất toàn bộ phần số liệu.** Agent chuẩn hóa (mới, không ngữ cảnh, cấm đọc results/docs/branch cũ): V2 = **177 s** (`rq2-agent-v2`, commit `146ca3b`), V3 = **137 s** (`rq2-agent-v3`, commit `6d62fe3`); diff cả hai lần trùng khớp từng dòng với `rq2-fix-v2/v3` → quy trình tái lập được. Đã điền vào `results/rq2-maintenance.md`, Chương 4 mục 4.3, panel 3 của `fig-rq2-maintenance.png`.
  - ⏳ Còn lại GĐ4 (việc của sinh viên): trình bày với GVHD theo `docs/gvhd-trao-doi.md` (5 mục); sau buổi gặp xử lý theo bảng "sau buổi gặp" trong file đó. Phần số liệu/hình/nháp Chương 4 đã đủ để bắt đầu GĐ5.
- [~] GĐ5: Báo cáo + slide + repo public (20/09) — **bắt đầu sớm 09/07**
  - ✅ 09/07: **Khung báo cáo dựng xong**: `docs/bao-cao/` — file điều phối `00-khung-bao-cao.md` (cấu trúc 5 chương + phụ lục A–E, quy ước viết, ánh xạ nguồn liệu, thứ tự viết đề xuất) + khung chi tiết Chương 1/2/3/5 (mỗi mục có ý chính + nguồn + checkbox); Chương 4 đã là nháp đầy đủ (chuyển từ `docs/chuong4-draft.md` sang `docs/bao-cao/chuong-4-ket-qua.md`).
  - ✅ 09/07 (tối): **Toàn bộ nháp báo cáo xong** — 5 chương văn xuôi đầy đủ + phụ lục A–E + tài liệu tham khảo [1]–[20] trong `docs/bao-cao/`. Chương 4 đã đánh số Hình 4.1–4.5 và có đoạn phân tích hòa vốn; còn đánh dấu: [HÌNH 3.1–3.3], [BẢNG 2.1], TODO 4.7-(3) (chờ GVHD).
  - ✅ 09/07 (tối): Hình 3.1 (`fig-design-pipeline.png` + script `make-pipeline-diagram.py`), Bảng 2.1 (Chương 2), deck PPTX (`slide-bao-ve.pptx` + `make-slides.py`), báo cáo Word (`bao-cao-doan.docx` + `make-report-docx.py`, 11 hình + 10 bảng nhúng sẵn) — tất cả sinh lại được bằng script.
  - ✅ 09/07 (tối): **chốt phương án (a) cho định vị baseline** — báo cáo khóa câu chữ "worst-case baseline" tại 4.7-(3) (lập luận: data-testid đòi sửa app; nhóm D không có nhãn ngữ nghĩa), baseline best-practice ghi ở hướng phát triển 5.4; mục 3 `gvhd-trao-doi.md` hạ xuống dạng thông báo. **Báo cáo hết TODO nội dung.**
  - ⏳ Còn lại GĐ5 (toàn bộ là việc con người, chi tiết `docs/bao-cao/00-khung-bao-cao.md`): sinh viên duyệt/sửa giọng văn từng chương (đối chiếu số liệu 4 bài ⭐ ở Chương 2); điền placeholder danh tính (bìa docx, slide 1, đề cương); ghép template khoa; gặp GVHD (thông báo 5 mục); demo + video dự phòng; chuyển repo public; xoay API key.
