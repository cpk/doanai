...
- [~] GĐ4: Thực nghiệm + phân tích (07/09) — **phần chạy tự động xong sớm 09/07**
  ...
  - ⏳ Còn lại GĐ4: (a) **RQ2 maintenance cost** theo **phương án lai** (chi tiết + lệnh tiếp tục: `docs/setup-new-machine.md` mục 2.1): Claude sửa locator V2/V3 trên branch riêng đo số-test-sửa + diff LOC (khách quan); chỉ số thời gian chờ chốt với GVHD (sinh viên tự sửa bấm giờ, hoặc bỏ + ghi Threats to Validity); VLM 0 test hỏng → 0 chi phí bảo trì. (b) phân tích + vẽ biểu đồ (mục 2.2 — không cần key); (c) nháp Chương 4 (mục 2.3).
