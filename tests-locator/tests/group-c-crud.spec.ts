...
test('C1: a created product survives a page reload (localStorage persistence)', async ({
  ...
  await page.reload();
  await expect(page.locator('.product-table tbody tr')).toHaveCount(13);
  await expect(page.locator('.product-table tbody tr:nth-child(13) td:nth-child(4)')).toHaveText(
    'SNK-004',
  );
});
...
test('C2: editing a product updates its row', async ({ page }) => {
  ...
  await page.click(`.product-table tbody tr:nth-child(2) ${ACTIONS_CELL} .icon-btn:nth-child(1)`);
  const modal = page.locator('.modal.product-form');
  ...
  await page.click("xpath=//div[contains(@class,'modal-actions')]/button[text()='Save']");
  await expect(page.locator('.product-table tbody tr:nth-child(2) td:nth-child(2)')).toHaveText(
    'Green Tea Bottle 600ml',
  );
  await expect(page.locator('.product-table tbody tr:nth-child(2) td:nth-child(5)')).toHaveText(
    '99',
  );
});
